...
**Entry point:** `src/app.py` defines Flask routes, page SEO metadata, sitemap/robots endpoints, and exposes `application = app` for WSGI servers.

**Routes:** `/`, `/experience/`, `/projects/`, `/blog/`, `/contact/`, `/health`, `/robots.txt`, `/sitemap.xml`.

**Data layer:** No database. Portfolio content lives in `src/db/` as JSON files.
